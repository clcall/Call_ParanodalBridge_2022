...
for c = 1:length(cond)
    ...
    for a = 1:length(an)
        ...
        elseif contains(cond{c},{'bsln','visL1'}) % work-around for d00 being empty in bsln struct
            tempdata00 = tempdata14;
        else
...
hold on
plotSpread(y,'distributionIdx',idx2,'distributionMarker','o','distributionColors',{ct,cu})
hold off
xlim([0 3])
ylim([0 0.15])
ax = gca();
...
y = vis.numbrgs./(vis.numbrgs + vis.numNonbrg);

avg = mean(y);
sem = calcSEM(y,1);

figure
plotSpread(y,'distributionMarker','o','distributionColors',[0.5 0.5 0.5])
hold on
errorbar(avg,sem,'ko','MarkerSize',4,'MarkerFaceColor','k','CapSize',0,'LineWidth',1.5);
hold off
xlim([0 2])
...
%% (VIS) Average lengths for all sheath types
colors2 = {[0.2 0.5 1],[52 75 160]./255,[1 0.5 0],[0.4 0.4 0.4]}; %,[1 0 0]
all = vis;
