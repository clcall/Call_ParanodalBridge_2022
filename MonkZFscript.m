...
xmlstruct = parseXML_SingleCell('fish07_cellE_bridgedSheath2_230508.traces');
% TO DO:
% file loop
% save struct
...
for i = 1:size(Dall,1)-1
    ...
    if edges_i(1) < edges_0i(1)
        wraps(edges_i(1):edges_0i(1)) = wraps(edges_i(1):edges_0i(1)) + repmat(mode(wraps(edges_0i(1)+50)),[1,edges_0i(1)-edges_i(1)+1]);
    end
    if edges_i(end) > edges_0i(end)
        wraps(edges_0i(end):edges_i(end)) = wraps(edges_0i(end):edges_i(end)) + repmat(mode(wraps(edges_0i(end)-50)),[1,edges_i(end)-edges_0i(end)+1]);
    end
