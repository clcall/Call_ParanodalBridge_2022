...
for c = 1:length(cond)
    ...
    vars = {'cellname','numbrgs','numNonbrg','avgbrglength','totbrglength'...
            'avgNonbrglength','avgChainLengthSum','cellTotLength',...
    ...
    for a = 1:length(an)
        ...
        if isempty(brgdata00)
            nonbrgdata = tempdata14.Sheath_Info((~brgIdx & ~contains(tempdata14.Sheath_Info.intNames, primNames)),:);
            ...
            data.(cond{c}).datatable = [data.(cond{c}).datatable; an(a),0,numNonbrg,NaN,NaN,avgNonbrglength,NaN,cellTotLength,NaN,avgNonBrgNodeRatio,NaN,NaN,NaN,NaN];
            continue
        end
        flag = [];
        primNames = cell(size(brgdata00,1),1);
        for i = 1:size(brgdata00,1)
            ...
            if size(procSO,1) < 1 
                % for whatever reason, bridge path is not connected to a primary sheath
                primdata14.intLengths = NaN;
                primdata14.node_class = NaN;
            elseif isempty(procSO{1})
                ...
                primIdx00 = cell2mat(cellfun(@(x) any(ismember(x,cell2mat(procSO))), intIDs, 'UniformOutput', false));
                primID00 = tempdata00.Sheath_Info.intIDs(primIdx00);
                primName00 = tempdata00.Sheath_Info.intNames(primIdx00);
                primdata14 = tempdata14.Sheath_Info(contains(tempdata14.Sheath_Info.intNames, primName00),:);
                primNames(i) = primName00;
            end
            if tempdata00.Sheath_Info.isbridgecnctd(primIdx00)
                for j = 1:size(brgdata14,1)
                    if contains(brgdata14.intNames{j},primName00)
                        if j < i % move last bridged sheath in chain to first position e.g. [3* 2* 1*]
                            ...
                            flag = [flag; j];
%                             flagprimdata{j} = [tempdata14.Sheath_Info.intLengths(primIdx00), tempdata14.Sheath_Info.node_class(primIdx00)];
                            flagbrgdata{j} = [brgdata14.intLengths(i), brgdata14.node_class(i)];
                        end
                    end
                end
            else % primary sheath is normal, assign bridged and primary lengths
                data.(cond{c}).(an{a}).lengths{i} = [brgdata14.intLengths(i), primdata14.intLengths(1)];
                data.(cond{c}).(an{a}).nodes{i} = [brgdata14.node_class(i), primdata14.node_class(1)];
                % add hasmltproc data
                data.(cond{c}).(an{a}).primMultProcs(i) = primdata14.hasmltprocs(1);
            end
            % ADD CASE FOR B-N-B
        ...
        primNames(cellfun(@isempty,primNames)) = [];
        nonbrgdata = tempdata14.Sheath_Info((~brgIdx & ~contains(tempdata14.Sheath_Info.intNames, primNames)),:);
...
%         cellTotNum = ALL.(cond{c}).(an{a}).num_FinalSheaths(end);
        ...
        data.(cond{c}).datatable = [data.(cond{c}).datatable; an(a),numbrgs,numNonbrg,avgbrglength,totbrglength,avgNonbrglength,avglengthSum,cellTotLength,avgChainNodeRatio,avgNonBrgNodeRatio,xy,z,xy_nb,z_nb];
    end
...
y = {bsln.avgbrglength, bsln.avgNonbrglength,...
     ctrl.avgbrglength, ctrl.avgNonbrglength};

plotEmUp(y,xes2,colors2)
xticklabels({})
figQuality(gcf,gca,[2 2.6])
y2 = [bsln.avgbrglength; bsln.avgNonbrglength;...
     ctrl.avgbrglength; ctrl.avgNonbrglength];
idx = [repmat({'bsln_b'},size(bsln.cellname)); repmat({'bsln_nb'},size(bsln.cellname));...
       repmat({'ctrl_b'},size(ctrl.cellname)); repmat({'ctrl_nb'},size(ctrl.cellname))];
kruskalwallis(y2,idx);
...
%% Chain neighbors VS normal sheath neighbors
% y = {bsln.avgChainNodeRatio, bsln.avgNonBrgNodeRatio,...
%     ctrl.avgChainNodeRatio, ctrl.avgNonBrgNodeRatio};
 
% plotEmUp(y,xes2,colors2)
% xticklabels({})
% figQuality(gcf,gca,[1.9 2.6])
% ytickformat('%.1f')
% y2 = [bsln.avgChainNodeRatio; bsln.avgNonBrgNodeRatio;...
%      ctrl.avgChainNodeRatio; ctrl.avgNonBrgNodeRatio];
% 
% [~,p,stat] = ttest2(bsln.avgChainNodeRatio, bsln.avgNonBrgNodeRatio);
% p*2
% [~,p,stat] = ttest2(ctrl.avgChainNodeRatio, ctrl.avgNonBrgNodeRatio);
% p*2
controls = [bsln;ctrl];
y = {controls.avgChainNodeRatio, controls.avgNonBrgNodeRatio};
avg = [mean(y{1},'omitnan'),mean(y{2},'omitnan')];
sem = [calcSEM(y{1},1),calcSEM(y{2},1)];
figure
[b,e] = errorbarbar(1,avg(1),sem(1),{'EdgeColor','none','FaceColor','none'},{'k','LineWidth',1.5,'LineStyle','none','MarkerSize',7,'MarkerFaceColor','k'});
e(1).Marker = 'o';
e(1).CapSize = 0;
e(1).MarkerFaceColor = 'k';
e(1).MarkerSize = 4;
hold on
[b,e] = errorbarbar(2,avg(2),sem(2),{'EdgeColor','none','FaceColor','none'},{'k','LineWidth',1.5,'LineStyle','none','MarkerSize',7,'MarkerFaceColor','k'});
e(1).Marker = 's';
e(1).MarkerFaceColor = 'k';
e(1).MarkerSize = 4.5;
e(1).CapSize = 0;
plotSpread(y,'distributionMarkers',{'o','s'},'distributionColors',{[0.5 0.5 0.5],[0.5 0.5 0.5]});
hold off
xticklabels({})
figQuality(gcf,gca,[1.8 2.6])
ytickformat('%.1f')
[~,p,tstat] = ttest2(controls.avgChainNodeRatio,controls.avgNonBrgNodeRatio)
...
multcompare(stats,"CType","dunn-sidak");
%% Chain neighbors VS normal sheath neighbors
y = {ctrl.avgChainNodeRatio, ctrl.avgNonBrgNodeRatio,...
    cupr.avgChainNodeRatio, cupr.avgNonBrgNodeRatio};

plotEmUp(y,xes2,colors2)
xticklabels({'Control' 'Remyelinating'})
figQuality(gcf,gca,[2.8 2.2])

y2 = [ctrl.avgChainNodeRatio; ctrl.avgNonBrgNodeRatio;...
     cupr.avgChainNodeRatio; cupr.avgNonBrgNodeRatio];

[~,p,stat] = ttest(ctrl.avgChainNodeRatio, ctrl.avgNonBrgNodeRatio);
p*2
[~,p,stat] = ttest(cupr.avgChainNodeRatio, cupr.avgNonBrgNodeRatio);
p*2
